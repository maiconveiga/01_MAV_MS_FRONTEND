...
const bufferRef = useRef<AlarmItem[]>([]);
  ...
  type ErrorEntry = {
    ...
    title: string;
    details: string;
    // metadata para identificar exatamente qual API falhou
    servidor?: string;
    ip?: string;
    versao?: string;
    baseUrl?: string;
    stage?: "login" | "collect" | "pipeline" | string;
  };
...
const COLLECTOR_HOST = import.meta.env.VITE_COLLECTOR_HOST || "localhost";
  ...
  try {
    ...
    const jobs = (catalog as CatalogApiLoose[]).map(api => limit(async () => {
      ...
      // pipeline: login -> coleta (com identificação por API e etapa)
      const baseUrl = api.BaseUrl;
      let token: string;
      try {
        token = await getTokenCached(baseUrl, api.Usuario, api.Senha);
      } catch (e: any) {
        if (ac.signal.aborted) return;
        addApiError({
          id: `${baseUrl}::login`,
  ...
  return (
    <div className="page">
        ...
        <div className="appbar__actions">
          ...
          {apiErrors.length === 0 ? (
            ...
            <button
              ...
              title="Clique para ver quais APIs não estão conectando"
            >
              ⚠️ {apiErrors.length} API(s) com falha
            </button>
          )}
        </div>
      </div>
      ...
      {/* Modal: Erros de conexão (lista) */}
      {errorsModalOpen && (
        <div className="modal" role="dialog" aria-modal="true" onClick={() => setErrorsModalOpen(false)}>
          <div className="modal__card" onClick={(e) => e.stopPropagation()}>
            <div className="modal__header">
              <div className="modal__title">
                <strong>Erros de conexão</strong>
                <div className="muted" style={{ marginTop: 4 }}>{apiErrors.length} API(s) com falha</div>
              </div>
              <button className="modal__close" onClick={() => setErrorsModalOpen(false)} aria-label="Fechar">✕</button>
            </div>
              ...
              <div className="tableCard" style={{ padding: 12 }}>
                <table className="table">
                  ...
                  <tbody>
                    {apiErrors.map((e) => (
                      <tr key={e.id}>
                        <td className="td">{e.servidor ?? "—"}</td>
                        <td className="td">{e.ip ?? "—"}</td>
                        <td className="td td--center">{e.versao ?? "—"}</td>
                        <td className="td">{e.baseUrl ?? "—"}</td>
                        <td className="td td--center">{e.stage ?? e.title}</td>
                        <td className="td" style={{ maxWidth: 520 }}>
                          <details>
                            <summary>{clip(e.details, 120)}</summary>
                            <div className="muted" style={{ whiteSpace: "pre-wrap", marginTop: 8 }}>{e.details}</div>
                          </details>
                        </td>
                      </tr>
                    ))}
                    {apiErrors.length === 0 && (
                      <tr><td className="td td--center" colSpan={6}>Sem erros.</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
              <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 12, gap: 8 }}>
                <button className="btn" onClick={() => setErrorsModalOpen(false)}>Fechar</button>
              </div>
            </div>
          </div>
        </div>
