...
const STATUSES = ["Não tratado","Em andamento","Concluído","Oportunidade"] as const;
...
export default function App() {
  ...
  // formulário API
  type ApiForm = {
    Servidor: string; IP: string; Usuario: string; offset: number; Versao: string; QuantidadeAlarmes: number; Senha: string;
  };
  const emptyForm: ApiForm = { Servidor: "", IP: "", Usuario: "", offset: 0, Versao: "V3", QuantidadeAlarmes: 1, Senha: "" };
  const [form, setForm] = useState<ApiForm>(emptyForm);
  const [editingId, setEditingId] = useState<string | null>(null);
  ...
  const [apiErrors, setApiErrors] = useState<ErrorEntry[]>([]);
  const [errorModal, setErrorModal] = useState<{ open: boolean; entry: ErrorEntry | null }>({ open: false, entry: null });
  const addApiError = useCallback((e: ErrorEntry) => setApiErrors(prev => (prev.some(x => x.id === e.id) ? prev : [...prev, e])), []);
  const clearApiErrors = useCallback(() => setApiErrors([]), []);

  // helpers tempo
  const occurrenceMsWithOffset = useCallback((it: AlarmItem) => {
    const base = (it as any).source_base_url ?? (it as any).__source_base_url ?? "";
    const off = offsetByBase[base] ?? 0;
    const ms = isoToMs((it as any).creationTime);
    return ms ? addHours(ms, off) : 0;
  ...
  const reload = useCallback(async () => {
    try {
      setLoading(true); setError(null); clearApiErrors();
      const catalog = await fetchCatalog();
      setCatalogLocal(catalog);

      const nameBy: Record<string, string> = {};
      const offBy: Record<string, number> = {};
      for (const a of catalog) { nameBy[a.BaseUrl] = a.Servidor; offBy[a.BaseUrl] = a.offset ?? 0; }
      setServerNameByBase(nameBy); setOffsetByBase(offBy);

      const aggregated: AlarmItem[] = [];
      for (const a of catalog) {
        const baseUrl = a.BaseUrl;
        let port = "";
        try { const u = new URL(baseUrl); port = u.port || (u.protocol === "https:" ? "443" : "80"); }
        catch {
          addApiError({ id: `${baseUrl}::url`, title: "Metasys IP inoperante", details: `BaseUrl inválida no catálogo: ${baseUrl}.` });
          continue;
        }
        ...
        try {
          ...
          const msg = (e?.message ?? String(e)).toLowerCase();
          if (msg.includes("failed to fetch") || msg.includes("network") || msg.includes("connect") || msg.includes("timeout")) {
            addApiError({ id: `${baseUrl}::collector-offline`, title: `API data colletor (${port}) offline`, details: `Não foi possível conectar ao Data Collector em ${baseUrl} (porta ${port}).`, port });
          } else {
            addApiError({ id: `${baseUrl}::login`, title: "Metasys IP inoperante", details: `Falha no login para ${baseUrl}.` });
          }
          continue;
        }
        ...
        // 2) coleta
        try {
          const session: Session = { base_url: baseUrl, token: token!, offset: a.offset ?? 0, servidor: a.Servidor, pageSize: Math.max(1, a.QuantidadeAlarmes || 1), page: 1, verify_ssl: false };
          const result = await collectAlarmsBatch([session]);
          aggregated.push(...result);
        } catch (e: any) {
          addApiError({ id: `${baseUrl}::collect`, title: "Metasys IP inoperante", details: `Falha ao coletar alarmes de ${baseUrl}.` });
        }
      }
      setItems(aggregated);
    } catch (e: any) {
  ...
  // selects base
  const uniqTipos = useMemo(() => {
    const s = new Set<string>(); for (const it of items) if (it.type) s.add(String(it.type)); return [...s].sort();
  }, [items]);
  const uniqServidores = useMemo(() => {
    const s = new Set<string>();
    for (const it of items) {
  ...
  }, [items, fServidores, fTipos, fNome, fItemRef, fMensagem, fValorTxt, fPriorMin, fPriorMax, fAck, fDesc, fInsDe, fInsAte, serverNameByBase, insertedMs]);
  ...
  const groupedRowsRaw = useMemo<GroupRow[]>(() => {
    ...
    for (const [key, arr] of map) {
      ...
      for (const it of arr) {
        const occ = insertedMs(it);
        if (occ > latestOcc) { latestOcc = occ; latest = it; }
        const p = num(it.priority) ?? -Infinity;
        if (p > highestPr) { highestPr = p; highest = it; }
      }
      const L = latest ?? arr[0];
      const H = highest ?? L;
      const base = (L as any).source_base_url ?? (L as any).__source_base_url ?? "";
  ...
  async function apiCreateComment(g: GroupRow, body: CommentIn) {
    const origin = commentsOriginForGroup(g);
    const r = await fetch(`${origin}/comments`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body) });
    const t = await r.text();
    if (!r.ok) throw new Error(`create ${r.status}: ${t || "sem corpo"}`);
    return JSON.parse(t) as Comment;
  }
  async function apiUpdateComment(g: GroupRow, id: string, patch: Partial<Pick<Comment, "text" | "status">>) {
    const origin = commentsOriginForGroup(g);
    const r = await fetch(`${origin}/comments/${encodeURIComponent(id)}`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify(patch) });
    const t = await r.text();
    if (!r.ok) throw new Error(`update ${r.status}: ${t || "sem corpo"}`);
    return JSON.parse(t) as Comment;
  }
  ...
  // carregar status/texto/horário (cache por TTL)
  useEffect(() => {
    let cancelled = false;
    const pairs: Array<{ g: GroupRow; ref: string }> = [];
    const now = Date.now();
    ...
    const runNext = async (): Promise<void> => {
      if (cancelled) return;
      const i = idx++; if (i >= pairs.length) return;
      const { g, ref } = pairs[i];
      try {
        const list = await apiListCommentsByReference(g, ref);
        if (cancelled) return;
        ...
        setLastStatusAtByRef(prev => ({ ...prev, [ref]: lastAt }));
        fetchedAtRef.current[ref] = Date.now();
      } catch { fetchedAtRef.current[ref] = Date.now(); }
      finally { void runNext(); }
    };
    for (let k = 0; k < Math.min(CONCURRENCY, pairs.length); k++) void runNext();
    return () => { cancelled = true; };
  }, [groupedRowsRaw]);
  ...
  useEffect(() => {
    ...
    const applyRules = async () => {
      for (const g of groupedRowsRaw) {
        ...
        if (normalized) {
          if (currentAuto !== "concluído" && currentAuto !== "concluido") setAutoTratativa(prev => ({ ...prev, [g.key]: "Concluído" }));
          continue;
        }
        const wasClosedOrWorking =
          ["concluído", "concluido", "em andamento", "oportunidade"].includes(currentPersisted) ||
          ["concluído", "concluido", "em andamento", "oportunidade"].includes(currentAuto);
        if (hasNewAlarm && wasClosedOrWorking) {
          if (currentAuto !== "não tratado") setAutoTratativa(prev => ({ ...prev, [g.key]: "Não tratado" }));
        }
      }
    };
    void applyRules();
  ...
  const statusShownForGroup = (g: GroupRow): Status => {
    const ref = g.itemReference ?? "";
    if (isNormalized(g)) return "Concluído";
    return (lastStatusByRef[ref] ?? autoTratativa[g.key] ?? "Não tratado") as Status;
  };
  ...
  const filteredTratOpts = useMemo(() => uniqTratativas.filter(s => s.toLowerCase().includes(tratSearch.toLowerCase())), [uniqTratativas, tratSearch]);
  ...
  const startEdit = (api: CatalogApi) => {
    const editing = getApiId(api);
    setEditingId(editing);
    setForm({ Servidor: api.Servidor ?? "", IP: api.IP ?? "", Usuario: api.Usuario ?? "", offset: Number(api.offset ?? 0), Versao: String(api.Versao ?? "V3"), QuantidadeAlarmes: Number(api.QuantidadeAlarmes ?? 1), Senha: api.Senha ?? "" });
  };
  const saveApi = async () => {
    try {
  ...
  const removeApi = async (id: string | null) => {
    ...
    try {
      ...
      await deleteApi(id);
      setCatalogLocal(await fetchCatalog());
      setApisNotice("API removida."); setTimeout(() => setApisNotice(null), 2500);
      await reload();
    } catch (e: any) {
      setApisError(e?.message ?? String(e));
    } finally { setApisBusy(false); }
  ...
  const testApi = async (api: CatalogApi) => {
    const id = getApiId(api) || `${api.Servidor}-${api.IP}-${api.Versao}`;
    setTestBusyById(prev => ({ ...prev, [id]: true })); setApisError(null); setApisNotice(null);
    try {
      ...
      setApisError(`Falha no teste de ${api.BaseUrl}: ${e?.message ?? String(e)}`);
    } finally { setTestBusyById(prev => ({ ...prev, [id]: false })); }
  };

  /* --------- Tratativa: modal e ações --------- */
  const [tratativaModal, setTratativaModal] = useState<{ open: boolean; group: GroupRow | null; comments: Comment[]; busy: boolean; error: string | null; text: string; status: string; editingId: string | null; }>({ open: false, group: null, comments: [], busy: false, error: null, text: "", status: "Não tratado", editingId: null });
  ...
  const openTratativaModal = async (g: GroupRow) => {
    ...
    try {
      ...
      if (comments[0]?.text) setLastTextByRef(prev => ({ ...prev, [reference]: String(comments[0].text) }));
      if (comments[0]?.created_at) setLastStatusAtByRef(prev => ({ ...prev, [reference]: isoToMs(comments[0].created_at) || 0 }));
      setTratativaModal(s => ({ ...s, open: true, group: g, comments, busy: false, error: null, text: "", status: isNormalized(g) ? "Concluído" : (comments[0]?.status ?? (autoTratativa[g.key] ?? "Não tratado")), editingId: null }));
    } catch (e: any) {
      setTratativaModal(s => ({ ...s, busy: false, error: e?.message ?? String(e) }));
    }
  };
  ...
  // autoscroll do wrapper quando o mouse se aproxima do topo/base
  const handleKanbanDragOver = (e: React.DragEvent) => {
    ...
    const y = e.clientY;
    const threshold = 56;             // zona ativa para rolar
    const speed = 24;                 // px por evento
  ...
  const onDropToStatus = async (e: React.DragEvent, newStatus: Status) => {
    e.preventDefault();
    const key = (e.dataTransfer.getData("text/plain") || draggingKey) as string | null;
    setDraggingKey(null);
    if (!key) return;
    const g = groupByKey.get(key);
    if (!g) return;
    try {
      await apiCreateComment(g, { reference: g.itemReference, text: "(alterado via kanban)", status: newStatus });
      setLastStatusByRef(prev => ({ ...prev, [g.itemReference]: newStatus }));
      setAutoTratativa(prev => ({ ...prev, [g.key]: newStatus }));
      setLastStatusAtByRef(prev => ({ ...prev, [g.itemReference]: Date.now() }));
    } catch (err: any) {
      alert(`Falha ao atualizar status: ${err?.message ?? String(err)}`);
    }
  };

  const KanbanColumn = ({ title, status }: { title: string; status: Status }) => {
    const cards = groupedSorted.filter(g => statusShownForGroup(g) === status);
    return (
      <div className="kanban__col" onDragOver={onDragOver} onDrop={(e) => onDropToStatus(e, status)}>
        ...
        <div className="kanban__colBody">
          {cards.map((g) => {
            const bell = computeBellColor(g);
            const val = g.value ?? ""; const un = g.units ?? "";
            return (
              <div
                key={g.key}
                className={`kanban__card ${draggingKey === g.key ? "kanban__card--dragging" : ""}`}
                draggable
                onDragStart={(e) => onDragStart(e, g.key)}
                title={`${g.name || g.itemReference}\n${g.message || ""}`}
              >
                <div className="kanban__cardHead">
                  <Bell color={bell} />
                  <span className="muted">{g.servidorName}</span>
                </div>
                <div className="kanban__cardTitle"><strong>{g.name || g.itemReference || "(sem nome)"}</strong></div>
                <div className="kanban__cardMeta">
                  <div className="muted">{g.itemReference}</div>
                  <div className="muted">{msToDMY(g.latestInsertedMs)}</div>
                </div>
                <div className="kanban__cardBody">
                  <div className="muted">{g.message}</div>
                  {(val || un) && <div className="pill">{String(val)} {String(un)}</div>}
                  {g.priority != null && <div className="pill">Prio: {g.priority}</div>}
                </div>
                <div className="kanban__cardActions">
                  <button className="btn btn--ghost" onClick={() => openTratativaModal(g)}>🗒️ Tratativa</button>
                  {g.servidorHref ? <a className="btn btn--ghost" href={g.servidorHref} target="_blank" rel="noreferrer">🔗 Abrir</a> : null}
                </div>
              </div>
            );
          })}
          {cards.length === 0 && <div className="muted" style={{ padding: 8 }}>Sem itens.</div>}
        </div>
      </div>
    );
  };
  ...
  return (
    <div className="page">
      {/* estilos mínimos (inclui wrapper rolável do kanban) */}
      <style>{`
        .kanbanWrap { max-height: calc(100vh - 170px); overflow: auto; }
        .kanban { display:flex; gap:12px; align-items:flex-start; padding:8px; min-width: 960px; }
        .kanban__col { flex:1; min-width:240px; background:#fafafa; border:1px solid #e5e7eb; border-radius:12px; overflow:hidden; }
        .kanban__colHeader { display:flex; justify-content:space-between; align-items:center; gap:8px; padding:10px 12px; background:#f3f4f6; border-bottom:1px solid #e5e7eb; position:sticky; top:0; z-index:1; }
        .kanban__colBody { padding:10px; display:grid; gap:10px; min-height:120px; }
        .kanban__card { background:#fff; border:1px solid #e5e7eb; border-radius:10px; padding:10px; box-shadow:0 1px 2px rgba(0,0,0,.04); cursor:grab; }
        .kanban__card--dragging { opacity:.6; box-shadow:0 6px 18px rgba(0,0,0,.15); }
        .kanban__cardHead { display:flex; align-items:center; gap:8px; margin-bottom:4px; }
        .kanban__cardTitle { margin:4px 0 6px; line-height:1.2; }
        .kanban__cardMeta { display:flex; justify-content:space-between; gap:8px; font-size:12px; margin-bottom:6px; }
        .kanban__cardBody { display:grid; gap:6px; }
        .kanban__cardActions { display:flex; gap:6px; margin-top:8px; }
        .pill { display:inline-block; padding:2px 8px; border:1px solid #e5e7eb; border-radius:999px; font-size:12px; }
        .badge { display:inline-flex; align-items:center; justify-content:center; min-width:24px; height:22px; padding:0 6px; border-radius:999px; border:1px solid #e5e7eb; background:#fff; font-size:12px; }
        .bell { display:inline-flex; margin-right:2px; }
        .bell--red svg path { fill:#ef4444; } .bell--gray svg path { fill:#9ca3af; } .bell--green svg path { fill:#10b981; } .bell--yellow svg path { fill:#f59e0b; }
      `}</style>
      ...
      <div className="appbar">
        ...
        <div className="appbar__actions" style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
          ...
          <button className="btn btn--secondary" onClick={() => setViewMode(viewMode === "table" ? "kanban" : "table")} title="Alternar modo de visualização">
      ...
      {viewMode === "table" ? (
          ...
          <div className="tableCard">
            <table className="table">
              <thead>
                ...
                <tr className="thead-filters">
                  <th className="th">
                    <button type="button" className="input" onClick={() => { setTempServidores([...fServidores]); setServerSearch(""); setServerModalOpen(true); }} title="Clique para filtrar por Servidor" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
                      <span style={{ fontWeight: 600, color: "var(--muted)" }}>Servidor:</span>
                      <span>{summarize(fServidores, uniqServidores.length)}</span>
                    </button>
                  </th>
                  ...
                  <th className="th">
                    <button type="button" className="input" onClick={() => { setTempTipos([...fTipos]); setTypeSearch(""); setTypeModalOpen(true); }} title="Clique para filtrar por Tipo" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
                      <span style={{ fontWeight: 600, color: "var(--muted)" }}>Tipo:</span>
                      <span>{summarize(fTipos, uniqTipos.length)}</span>
                    </button>
                  </th>
                  ...
                  <th className="th">
                    <button type="button" className="input" onClick={() => { setTempTrat([...fTratativas]); setTratSearch(""); setTratModalOpen(true); }} title="Clique para filtrar por Tratativa" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
                      <span style={{ fontWeight: 600, color: "var(--muted)" }}>Tratativa:</span>
                      <span>{summarize(fTratativas, uniqTratativas.length)}</span>
                    </button>
                  </th>
              ...
              <tbody>
                {groupedSorted.map((g) => {
                  const val = g.value ?? ""; const un = g.units ?? "";
                  const link = g.servidorHref; const bell = computeBellColor(g);
                  const status = statusShownForGroup(g);
                  return (
                    <tr key={g.key}>
                      <td className="td td--servidor">
                        <span title="status"><Bell color={bell} /></span>
                        {link ? <a href={link} target="_blank" rel="noreferrer" className="link" title={g.servidorName}>{g.servidorName || "—"}</a> : (g.servidorName || "—")}
                      </td>
                      <td className="td">
                        <strong title={g.name || ""}>{g.name || g.itemReference || "(sem nome)"}</strong>{" "}
                        {g.items.length > 1 && <button className="btn btn--ghost badge" title={`Ver ${g.items.length} alarmes do grupo`} onClick={() => setModalGroup(g)}>x{g.items.length}</button>}
                      </td>
                      <td className="td" title={g.itemReference}>{g.itemReference || "—"}</td>
                      <td className="td" title={g.message}>{g.message ?? ""}</td>
                      <td className="td">{String(val)} {String(un)}</td>
                ...
                {!loading && groupedSorted.length === 0 && <tr><td className="td td--center" colSpan={11}>Nenhum item encontrado com os filtros atuais.</td></tr>}
              </tbody>
            </table>
          </div>
          {error && <div className="error" style={{ marginTop: 12 }}>Erro: {error}</div>}
        ...
        /* =================== KANBAN =================== */
        <div className="kanbanWrap" ref={kanbanWrapRef} onDragOver={handleKanbanDragOver}>
          <div className="kanban">
            <KanbanColumn title="Não tratado" status="Não tratado" />
            <KanbanColumn title="Em andamento" status="Em andamento" />
            <KanbanColumn title="Concluído" status="Concluído" />
            <KanbanColumn title="Oportunidade" status="Oportunidade" />
          </div>
        </div>
      )}
            ...
            <div className="modal__body">
              <div className="tableCard">
                <table className="table">
                  ...
                  <tbody>
                    {[...modalGroup.items].sort((a, b) => insertedMs(b) - insertedMs(a)).map((it, idx) => {
                      const base = (it as any).source_base_url ?? (it as any).__source_base_url ?? "";
                      const srv = serverNameByBase[base] ?? base.replace(/^https?:\/\//, "");
                      const v = (it as any).triggerValue_value ?? (it as any).triggerValue?.value ?? "";
                      const u = (it as any).triggerValue_units ?? (it as any).triggerValue?.units ?? "";
        ...
        <div className="modal" role="dialog" aria-modal="true" onClick={() => setApisOpen(false)}>
          <div className="modal__card" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal__body">
              ...
              <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
                {editingId ? (<><button className="btn btn--primary" disabled={apisBusy} onClick={saveApi}>💾 Salvar</button><button className="btn" disabled={apisBusy} onClick={startCreate}>➕ Novo</button></>) : (<button className="btn btn--primary" disabled={apisBusy} onClick={saveApi}>➕ Cadastrar</button>)}
              </div>
              ...
              <div className="tableCard">
                <table className="table">
                  ...
                  <tbody>
                    ...
                    {catalogLocal.length === 0 && <tr><td className="td td--center" colSpan={10}>Nenhuma API cadastrada.</td></tr>}
                  </tbody>
                </table>
              </div>
            </div>
      ...
      {errorModal.open && errorModal.entry && (
        <div className="modal" role="dialog" aria-modal="true" onClick={() => setErrorModal({ open: false, entry: null })}>
          <div className="modal__card" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal__body">
              <div className="tableCard" style={{ padding: 12 }}><div className="muted" style={{ whiteSpace: "pre-wrap" }}>{errorModal.entry.details}</div></div>
              <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 12, gap: 8 }}>
                <button className="btn" onClick={() => setErrorModal({ open: false, entry: null })}>Fechar</button>
              </div>
            </div>
      ...
      {tratativaModal.open && tratativaModal.group && (
        <div className="modal" role="dialog" aria-modal="true" onClick={() => setTratativaModal(s => ({ ...s, open: false }))}>
          <div className="modal__card" onClick={(e) => e.stopPropagation()}>
            <div className="modal__header">
              <div className="modal__title">
                <strong>Tratativa — {tratativaModal.group.name || tratativaModal.group.itemReference}</strong>
                <div className="muted" style={{ marginTop: 4 }}>{tratativaModal.group.itemReference}</div>
              </div>
              <button className="modal__close" onClick={() => setTratativaModal(s => ({ ...s, open: false }))} aria-label="Fechar">✕</button>
            </div>
            <div className="modal__body">
              {tratativaModal.error && <div className="error" style={{ marginBottom: 8 }}>Erro: {tratativaModal.error}</div>}
              <div className="tableCard" style={{ marginBottom: 12 }}>
                <table className="table">
                  <thead>
                    <tr className="thead-title">
                      ...
                      <th className="th"><span className="th__inner"><span>Comentário</span></span></th>
                      <th className="th"><span className="th__inner"><span>Ações</span></span></th>
                    </tr>
                  </thead>
                  <tbody>
                    {tratativaModal.comments.map((c) => (
                      <tr key={c.id}>
                        <td className="td">{c.created_at}</td>
                        <td className="td">{c.status ?? "—"}</td>
                        <td className="td">{c.text}</td>
                        <td className="td td--nowrap">
                          <button className="btn btn--ghost" onClick={() => onEditComment(c)}>✏️ Editar</button>{" "}
                          <button className="btn btn--ghost" onClick={() => onDeleteComment(c)}>🗑️ Excluir</button>
                        </td>
                      </tr>
                    ))}
                    {tratativaModal.comments.length === 0 && <tr><td className="td td--center" colSpan={4}>Sem comentários.</td></tr>}
                  </tbody>
                </table>
              </div>
              <div style={{ display:"grid", gridTemplateColumns:"1fr 220px", gap:8, alignItems:"start" }}>
                <textarea className="input" placeholder="Escreva um comentário (máx. 255)" maxLength={255} rows={3} value={tratativaModal.text} onChange={e => setTratativaModal(s => ({ ...s, text: e.target.value }))} />
                <div style={{ display:"grid", gap:8 }}>
                  <select className="input select" value={tratativaModal.status} onChange={e => setTratativaModal(s => ({ ...s, status: e.target.value }))}>
                    {STATUSES.map(o => <option key={o} value={o}>{o}</option>)}
                  </select>
                  <button className="btn btn--primary" disabled={tratativaModal.busy || (!tratativaModal.text && !tratativaModal.editingId)} onClick={onSaveTratativa} title={tratativaModal.editingId ? "Salvar alterações" : "Adicionar comentário"}>
                    {tratativaModal.editingId ? "💾 Atualizar" : "➕ Adicionar"}
                  </button>
                </div>
              </div>
              {tratativaModal.busy && <div className="muted" style={{ marginTop: 8 }}>Processando…</div>}
            </div>
          </div>
        </div>
      )}
